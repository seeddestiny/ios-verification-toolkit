...
> - 安装 XCUITest 驱动后，脚本会把同一个动态 Bundle ID 写入本机 `~/.appium` 下的 `WebDriverAgentRunner` Debug/Release 配置，再进行脚本签名或打开 Xcode。这样命令行构建、Xcode 手动签名和 Appium session 始终使用同一个 ID；不会修改业务工程。
...
| 2 | WDA 签名构建到真机 | 先由脚本无人值守签名；失败后用本轮选中的 Xcode 自动打开 WDA 工程，由用户在 Xcode 手动选择 Team、真机并完成一次签名构建 |
