...
- **默认安全、显式放宽**：拒绝 URL 内嵌凭据、未授权 HTTP 和多个 PyPI 源；需要降低限制时必须由用户明确选择。
...
| 顺序 | npm | PyPI |
|---:|---|---|
| 1 | `npm_config_registry` / `NPM_CONFIG_REGISTRY` | `PIP_INDEX_URL` |
| 2 | npm 按自身规则解析的项目、用户或全局 `.npmrc` | pip 的本机 `global.index-url` / `install.index-url` |
| 3 | `IOS_MCP_NPM_REGISTRY`，仅作项目级后备 | `IOS_MCP_PYPI`，仅作项目级后备 |
| 4 | `https://registry.npmjs.org/` | `https://pypi.org/simple` |

前两级属于用户现有配置，普通安装无需重复设置。`npm_config_registry` 和 `PIP_INDEX_URL` 是标准环境变量，只影响当前进程及子进程；`IOS_MCP_NPM_REGISTRY` 和 `IOS_MCP_PYPI` 才是本项目定义的后备变量。若用户本机已经配置自己的源，项目后备值和官方默认值都不会覆盖它。
...
> - Xcode 选择顺序为：本轮显式 `DEVELOPER_DIR` → 有效且包含 `xcodebuild` 的 `xcode-select` → 本机唯一一份完整 Xcode。解析结果只导出给当前安装/MCP 进程，不执行 `sudo xcode-select --switch`，也不修改其它应用使用的系统全局配置；安装多份完整 Xcode时才要求显式选择。
...
- **供应链**:npm 与 pip 都优先使用用户环境或本机当前有效配置，否则使用各自官方源；仓库不内置公司 npm/PyPI 域名。自定义 npm 源缺包时必须通过 `ALLOW_PUBLIC_NPM=1` 才能为驱动安装临时回退官方源。pip 始终只使用单一 index，不接受 `extra-index-url` 或 URL 内嵌凭据。Appium 不使用 sudo 全局安装，MCP Python 依赖装在隔离 venv。
