...
def resolve_developer_dir(
    ...
    *,
    ...
    explicit = env.get("DEVELOPER_DIR", "").strip()
    if explicit:
    ...
    if candidates is None:
        ...
        discovered.extend(Path(item) for item in glob.glob(str(Path.home() / "Applications/Xcode*.app/Contents/Developer")))
    else:
        discovered = list(candidates)
    valid = sorted({path.expanduser().resolve() for path in discovered if _valid_developer_dir(path.expanduser().resolve(), required_tool)})
    if len(valid) == 1:
        return valid[0]
    if not valid:
        raise RuntimeError(f"未找到包含 {required_tool} 的完整 Xcode；请通过 DEVELOPER_DIR 显式指定")
    raise RuntimeError(
        f"发现多个可用 Xcode，无法安全选择；请设置 DEVELOPER_DIR。候选: {', '.join(str(path) for path in valid)}"
    )
